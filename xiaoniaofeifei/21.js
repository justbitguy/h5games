...
(function() {
	...
	Array.prototype.indexOfAttr = function(a, c) {
		for (var i = (typeof a).toLowerCase(), v = -1, b = 0,
				d = this.length; b < d; b++)
			if ("string" == i && this[b][a] == c || "number" == i && this[b] == a) {
				v = b;
				break
			}
		return v
	...
	var j = "linkScreen",
		...
		V, M, W, N, m = jsGame = link = {
			...
			clone: function(a) {
				...
				if ("object" == typeof c)
					if (void 0 != c.length)
						for (var a = [], i = 0, b = c.length; i < b; i++) void 0 !== c[i] && (a[i] = null != c[i] && "object" == typeof c[i] ? void 0 != c[i].length ? c[i].slice(0) : c[i] : c[i]);
					else
						for (i in a = {},
			...
			canvas: {
				...
				drawImage: function(a, c, i, b, d, e, g, h, f, q) {
					...
					else if (null != j.src) null != h && (c = 0 > c ? 0 : c, h = 0 >= h ? 0.1 : h), null != f && (i = 0 > i ? 0 : i, f = 0 >= f ? 0.1 : f), null != b && null != h && (b = 0 >= b ? 0.1 : c + b <= j.width ? b : j.width - c), null != d && null != f && (d = 0 >= d ? 0.1 : i + d <= j.height ? d : j.height - i), j.loaded && (b ? d ? q ? (G = l.getAnchor(e, g, h, f, q), k.drawImage(j, c, i, b, d, G.x,
						G.y, h, f)) : k.drawImage(j, c, i, b, d, e, g, h, f) : (G = l.getAnchor(c, i, j.width, j.height, b), k.drawImage(j, G.x, G.y)) : k.drawImage(j, c, i));
					else if ((a = K[a]) && !a.inited) l.setImage(a.id, a.src, a.benchId, a.bench, a.cache), a.inited = !0;
					return this
				},
				...
				drawRegion: function(a, c, b, d, e, g, h, f) {
					switch (g) {
						default: k.transform(1, 0, 0, 1, h, f);
						break;
						case 5:
								k.transform(0,
								1, -1, 0, e + h, f);
				...
				drawRegionAndZoom: function(a, c, b, d, e, h, g, f, j, q, n) {
					switch (h) {
						default: k.transform(1, 0, 0, 1, g, f);
						break;
						case 5:
								k.transform(0, 1, -1,
								0, n + g, f);
				...
				drawNumber: function(a, c, b, d, e, g, f, h, j) {
					...
					if ("center" == f) {
						...
						for (f = 0; f < q; f++) this.drawImage(c, parseInt(a.charAt(f)) * b, 0, b, d, e + f * h, g, h, j);
					else if (!1 == f)
						for (f = q - 1; 0 <= f; f--) this.drawImage(c, parseInt(a.charAt(f)) * b, 0, b, d, e - (q - 1 - f) * h, g, h, j, m.graphics.ANCHOR_RT);
					return this
				},
				moveTo: function(a, c) {
					k.moveTo(a, c);
			...
			clearAsyncImageCache: function() {
				try {
					...
					for (b in u)
						if (a = u[b])
							if (c = K[b]) c.inited = !1, this.delImage(b).canvas.del(b)
				} catch (d) {}
				return this
			},
			audio: {
	...
	link.classes.Observer.prototype.register = function(a) {
		if (null == a) return this; - 1 == m.comm.inArray(a, this.group) && this.group.push(a);
		return this
	};
	link.classes.Observer.prototype.unregister = function(a) {
		if (null == a) return this;
		a = m.commandFuns.inArray(a, this.group); - 1 < a && this.group.splice(a, 1);
		return this
	};
...
(function(g) {
	...
	link.action.Role.prototype.setSprite =
		function(b, e, n) {
			...
			if (this.links)
				for (h = 0; j = this.links[h]; h++) j.setSprite(b, e, n);
			trans = null;
			return this
		};
	...
	link.action.Role.prototype.action = function() {
		...
		if (e) {
			...
			this.aR = g.aR;
			this.bR = g.bR;
			if (0 < this._path.length) j = this._path.shift(), h = j[0] || 0, j = j[1] || 0, this._skipMoveDs || this.setSprite(this._moveDs[B(h, j)], !0), this.svx = h, this.svy = j, this.mapOffx += this.svx, this.mapOffy += this.svy;
	...
	link.action.Role.prototype.render = function() {
		...
		if (b && this._fA) {
			var e = this._fA,
				...
				b = b.trans,
				j, p, s;
			e[0] && (j = this.x, p = this.y, g.canvas.save().translate(j, p).rotate(this.angle * Math.PI / 180).translate(-j, -p));
		...
		var B = g.canvas.screen.getDevice(),
			x = {
				...
				setBg: function(b) {
					this.bgType = b;
					switch (this.bgType) {
						default: b = "bg1";this.bgColor = "#4EC0CA";
						break;
						case 2:
								b = "bg2",
							this.bgColor = "#038794"
				...
				birdAction: function() {
					if (this.bird) switch (this.bird.action(), this.bird.state) {
						case "waiting":
							this.bird.endPath() && this.bird.setPath([
								[0, -1.5],
								[0, -1.5],
								[0, -1.5],
								[0, -1.5],
